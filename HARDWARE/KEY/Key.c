...
{
		return GPIO_ReadInputDataBit(GPIOX,GPIO_Pin_W);
}
...
void KEY_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin  =GPIO_Pin_X | GPIO_Pin_Y | GPIO_Pin_Z | GPIO_Pin_W;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //PA6设置成输入，默认下拉	  
	GPIO_Init(GPIOX, &GPIO_InitStructure);//初始化GPIOA.6201
}
...
int8_t Get_Key(void)
{
	...
	else if(GPIO_ReadInputDataBit(GPIOX,GPIO_Pin_Y) == 0){
		delay_ms(10);
		while(GPIO_ReadInputDataBit(GPIOX,GPIO_Pin_Y) == 0);
		delay_ms(10);
		key = KEY2;
	}
	else if(GPIO_ReadInputDataBit(GPIOX,GPIO_Pin_Z) == 0){
		delay_ms(10);
		while(GPIO_ReadInputDataBit(GPIOX,GPIO_Pin_Z) == 0);
		delay_ms(10);
		key = KEY3;
	}
	return key;
...
{
	if(Get_OS_Time() - old_open_time > open_time * 1000){
		open_flage = 1;
		open_time += 10;
	}
	else{
